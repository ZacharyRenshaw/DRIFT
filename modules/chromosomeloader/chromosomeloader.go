...
func LoadChromosomeArmsFromCSV(model *types.Model){
   ...
   var totallen int
	...
	for _, record := range records[1:] {
      ...
      if model.ChromosomeArms[chromosome]      == nil { model.ChromosomeArms[chromosome] = make(map[int][]int) }
      if model.ChromosomeArms[chromosome][arm] == nil { model.ChromosomeArms[chromosome][arm] = make([]int, 2) }
